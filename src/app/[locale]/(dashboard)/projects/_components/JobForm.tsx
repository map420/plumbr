...
export function JobForm({ translations: t, job, clients = [], preselectedClientId }: { translations: FormTranslations; job?: Job; clients?: Client[]; preselectedClientId?: string }) {
  ...
  const initialClientId = job?.clientId ?? preselectedClientId ?? null
  ...
  const [form, setForm] = useState({
    name: job?.name ?? '',
    clientName: job?.clientName ?? initialClient?.name ?? '',
    clientEmail: job?.clientEmail ?? initialClient?.email ?? '',
    clientPhone: job?.clientPhone ?? initialClient?.phone ?? '',
    address: job?.address ?? initialClient?.address ?? '',
    ...
    startTransition(async () => {
      try {
        ...
        if (job) {
          ...
          setSaved(true)
          router.push(`/${locale}/projects/${job.id}`)
        } else {
  ...
  return (
    <>
    {saved && <Toast message="Project saved successfully!" onDone={() => setSaved(false)} />}
    <form onSubmit={handleSubmit} className="bg-card md:bg-transparent min-h-full max-w-3xl">
      ...
      <div className="flex items-center px-4 py-2.5 md:hidden" style={{ borderBottom: '1px solid var(--wp-border-light)' }}>
        ...
        <span className="flex-shrink-0" style={{ fontSize: '0.9375rem', fontWeight: 700, color: 'var(--wp-text-primary)', lineHeight: '1.25rem' }}>{job ? 'Edit Project' : 'New Project'}</span>
        ...
        {clients.length > 0 && (
          <div ref={dropdownRef} className="relative">
            ...
            {showDropdown && !selectedClientId && filteredClients.length > 0 && (
              <div className="absolute z-10 w-full mt-1 bg-card rounded-lg max-h-48 overflow-y-auto" style={{ border: '1px solid var(--wp-border)', boxShadow: 'var(--wp-shadow-lg)' }}>
