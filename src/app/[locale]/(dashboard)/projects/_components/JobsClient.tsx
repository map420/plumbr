...
export function JobsClient({ initialJobs, jobAssignments, planInfo, translations: t }: { initialJobs: Job[]; jobAssignments: Assignment[]; planInfo: { current: number; limit: number } | null; translations: Translations }) {
  ...
  const grouped = filtered.reduce<Record<string, Job[]>>((acc, job) => {
    ...
    const key = date.toLocaleDateString(locale === 'es' ? 'es-ES' : 'en-US', { year: 'numeric', month: 'long' })
    if (!acc[key]) acc[key] = []
    acc[key].push(job)
    return acc
  }, {})
  ...
  function handleDelete() {
    if (!deleteId) return
    const name = initialJobs.find(j => j.id === deleteId)?.name ?? 'Project'
  ...
  return (
    <div className="px-4 pt-2 pb-4 md:p-8 bg-card md:bg-transparent min-h-full">
        ...
        <div className="flex items-center gap-3">
          <div className="flex rounded-lg overflow-hidden" style={{ border: '1px solid var(--wp-border-v2)' }}>
            <button onClick={() => setViewMode('kanban')} className="text-xs px-3 py-1.5 font-medium"
              style={{ background: viewMode === 'kanban' ? 'var(--wp-brand)' : 'var(--wp-surface)', color: viewMode === 'kanban' ? 'var(--wp-text-inverse)' : 'var(--wp-text-3)' }}>
              Kanban
            </button>
            <button onClick={() => setViewMode('list')} className="text-xs px-3 py-1.5 font-medium"
              style={{ background: viewMode === 'list' ? 'var(--wp-brand)' : 'var(--wp-surface)', color: viewMode === 'list' ? 'var(--wp-text-inverse)' : 'var(--wp-text-3)' }}>
              List
            </button>
      ...
      <div className="md:hidden">
        <input type="text"
          placeholder={locale === 'es' ? 'Buscar proyectos...' : 'Search by project or client...'}
      ...
      <div className="hidden md:block">
        <Toolbar
          ...
          searchPlaceholder={locale === 'es' ? 'Buscar por proyecto o cliente...' : 'Search by project or client...'}
          right={
            <Segmented
              value={filter}
              onChange={setFilter}
      ...
      {initialJobs.length === 0 ? (
        ...
        <div className="py-12 text-center" style={{ color: 'var(--wp-text-3)', fontSize: '0.875rem' }}>
          {locale === 'es' ? 'Sin resultados.' : 'No projects match your search.'}
        </div>
      ) : (
        <>
          {/* Mobile list */}
            ...
            {monthKeys.map((month, mi) => {
              ...
              return (
                <div key={month}>
                  ...
                  {items.map((job, idx) => (
                    <div key={job.id} onClick={() => router.push(`/${locale}/projects/${job.id}`)}
                      onTouchStart={() => router.prefetch(`/${locale}/projects/${job.id}`)}
                      onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); router.push(`/${locale}/projects/${job.id}`) } }}
                      role="link"
                      tabIndex={0}
                      aria-label={`Open project ${job.clientName}`}
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-baseline justify-between gap-2">
                          <Link href={`/${locale}/projects/${job.id}`} className="truncate hover:underline" style={{ fontSize: '0.9375rem', fontWeight: 600, color: 'var(--wp-text)' }}>{job.name}</Link>
                          {job.budgetedCost && parseFloat(job.budgetedCost) > 0 && (
                            <span className="text-price shrink-0" style={{ fontSize: '0.9375rem', color: 'var(--wp-text)' }}>
                              ${parseFloat(job.budgetedCost).toLocaleString(locale === 'es' ? 'es-ES' : 'en-US')}
                            </span>
                          )}
                        </div>
                        <div className="flex items-center justify-between mt-1">
                          <span style={{ fontSize: '0.75rem', color: 'var(--wp-text-3)' }}>
                            {job.clientName}
                            {job.startDate && <> · {new Date(job.startDate).toLocaleDateString(locale === 'es' ? 'es-ES' : 'en-US', { month: 'short', day: 'numeric' })}</>}
                          </span>
                          {(() => {
                            const derived = deriveJobStatus(job)
                            const label = derived === 'scheduled' ? 'Scheduled' : t.status[derived as JobStatus]
                            return <JobStatusBadge status={derived} label={label} />
                          })()}
                        </div>
                      </div>
                    </div>
                        ...
                        return (
                          <Link key={job.id} href={`/${locale}/projects/${job.id}`}
                            ...
                            <div className="flex items-center justify-between mt-2">
                              {job.startDate && <span className="text-[10px]" style={{ color: 'var(--wp-text-3)' }}>{new Date(job.startDate).toLocaleDateString(locale === 'es' ? 'es-ES' : 'en-US', { month: 'short', day: 'numeric' })}</span>}
                              {job.budgetedCost && parseFloat(job.budgetedCost) > 0 && (
                                <span className="text-xs font-semibold tabular-nums" style={{ color: 'var(--wp-text)' }}>${parseFloat(job.budgetedCost).toLocaleString(locale === 'es' ? 'es-ES' : 'en-US')}</span>
                              )}
                            </div>
                            {/* Progress bar */}
                            {status !== 'lead' && (
              ...
              <table className="w-full text-sm min-w-[720px]">
                ...
                <tbody>
                  {filtered.map((job) => (
                    <tr
                      key={job.id}
                      onClick={() => router.push(`/${locale}/projects/${job.id}`)}
                      onMouseEnter={() => router.prefetch(`/${locale}/projects/${job.id}`)}
                    ...
                    >
                      ...
                      <td className="px-4 py-3" style={{ color: 'var(--wp-text-2)' }}>
                        {job.startDate ? new Date(job.startDate).toLocaleDateString(locale === 'es' ? 'es-ES' : 'en-US') : '—'}
                      </td>
                      <td className="px-4 py-3 text-right tabular-nums font-semibold" style={{ color: 'var(--wp-text)' }}>
                        {job.budgetedCost && parseFloat(job.budgetedCost) > 0 ? `$${parseFloat(job.budgetedCost).toLocaleString(locale === 'es' ? 'es-ES' : 'en-US')}` : '—'}
                      </td>
                      <td className="px-4 py-3">
