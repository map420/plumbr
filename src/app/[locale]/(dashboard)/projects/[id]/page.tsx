...
import { getJobChecklistItems } from '@/lib/actions/job-checklists'
import { getActiveClockSession } from '@/lib/actions/clock'
import { JobDetailClient } from '../_components/JobDetailClient'

export default async function JobDetailPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = await params
  const [tj, tc, te, ti, job, estimates, invoices, expenses, allTechnicians, assignedTechnicians, photos, changeOrders, workOrders, shoppingLists, checklistItems, activeSession] = await Promise.all([
    ...
    getJobChecklistItems(id).catch(() => []),
    getActiveClockSession(id).catch(() => null),
  ])
  if (!job) notFound()
