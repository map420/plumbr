...
export default async function JobDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [tj, tc, te, ti, job, estimates, invoices] = await Promise.all([
    getTranslations('jobs'), getTranslations('common'), getTranslations('estimates'), getTranslations('invoices'),
    getJob(id), getEstimatesByJob(id), getInvoicesByJob(id),
  ])
  if (!job) notFound()

  return (
    <JobDetailClient
      job={job} estimates={estimates} invoices={invoices}
      translations={{
