...
export default async function FieldJobPage({ params }: { params: Promise<{ projectId: string; locale: string }> }) {
  const { projectId, locale } = await params
  const [t, tj, job, workOrders] = await Promise.all([
    getTranslations('field'), getTranslations('jobs'), getJob(projectId), getWorkOrdersByJob(projectId),
  ])
  if (!job) notFound()
